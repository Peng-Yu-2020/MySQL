...
FROM sales_order join sales_item on sales_order.id = sales_item.sales_order_id
JOIN item
on item.id = sales_item.item_id
...
sales_person.first_name AS SalesFName, 
sales_person.last_name  AS SalesLName
FROM sales_order
JOIN sales_item
ON sales_item.sales_order_id = sales_order.id
JOIN item
